...
// --- Estrutura auxiliar para armazenar distância e classe ---
...
float euclidean_distance(const Sample &a, float c, float d, float h, float r, float s, float e)
{
    float dx = a.contrast - c;
    float dd = a.dissimilarity - d;
    float dh = a.homogeneity - h;
    float dr = a.correlation - r;
    float ds = a.std_dev - s;
    float de = a.entropy - e;
    return sqrt(dx * dx + dd * dd + dh * dh + dr * dr + ds * ds + de * de);
}

// static DistanceLabel dist[DATASET_SIZE];
// --- Inferência KNN ---
int infer_knn(float c, float d, float h, float r, float s, float e)
{
    DistanceLabel* dist = (DistanceLabel*)heap_caps_malloc(sizeof(DistanceLabel) * DATASET_SIZE, MALLOC_CAP_SPIRAM);
    if (!dist) {
        // fallback
        dist = (DistanceLabel*)malloc(sizeof(DistanceLabel) * DATASET_SIZE);
        if (!dist) {
            // erro fatal
            return -1;
        }
    ...
    {
        dist[i].distance = euclidean_distance(dataset[i], c, d, h, r, s, e);
        dist[i].label = dataset[i].alert;
    }

    // Ordena por distância (bubble sort simples)
...
void myHeavyTask(void *param)
{
    ...
    while (true)
    {
        ...
        // Chamar função da KNN para inferência
        int class_label = infer_knn(contrast, energy, homogeneity, correlation, std_dev, entropy);
...
void setup()
{
    ...
    // --- Exemplo: atributos extraídos (substituir por reais!) ---
    float contrast = 0.48;
    float dissimilarity = 0.27;
    float homogeneity = 0.66;
    float correlation = 0.28;
    float std_dev = 0.21;
    float entropy = 0.12;
